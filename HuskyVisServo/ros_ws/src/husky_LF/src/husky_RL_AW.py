...
model = PPO.load("/home/HuskyVisServo/trained_models/KappaPolicy/best25",device = 'cuda')
model2 = PPO.load("/home/HuskyVisServo/trained_models/cone",device = 'cuda')
...
class LineFollower(object):
    ...
    def camera_callback(self, data):
        ...
        ## RL Cone Policy
        imgNN_RLCone = cv_image[288:480, 0:640]
        imgNN_RLCone = cv2.resize(imgNN_RLCone, (0,0), fx=0.5, fy=0.5)
        #cv2.imshow("Cropped", imgNN_RLCone)
        ...
        hsv2 = cv2.cvtColor(imgNN_RLCone, cv2.COLOR_BGR2HSV)
        ...
        im_bw2 = cv2.inRange(hsv2, lower_orange, upper_orange)
        ...
        im_bw = np.frombuffer(im_bw, dtype=np.uint8).reshape(96, 320, 1) # Reshape to required observation size
        im_bw2 = np.frombuffer(im_bw2, dtype=np.uint8).reshape(96, 320, 1) # Reshape to required observation size
        im_bw_obs = im_bw
        im_bw_obs2 = im_bw2
        #cv2.imshow("Input", im_bw_obs)
        #cv2.waitKey(1)
        inputs = np.array(im_bw,dtype = np.uint8)
        inputs2 = np.array(im_bw2,dtype = np.uint8)
        ...
        self.enc_len_div = np.clip(self.enc_len,0.01,4)
        self.enc_len_div = 63.75*self.enc_len
        #self.obs_ = np.divide(self.obs_,self.enc_len_div)
        ...
        cv2.imshow("Input", self.obs_)
        cv2.waitKey(1)
        		

        pred = model.policy.predict(self.obs_)
        pred2 = model2.policy.predict(self.obs2_)
        
        a1= pred[0]
        a2= pred2[0]
        V_pred1 = 0.25*a1[0].item() + 0.75 # >> Constrain to [0.1 0.9] >> Complete space 0 -> 1  
        self.V1 = V_pred1
        ...
        omega_pred1 = V_pred1*a1[1].item()#Omega range : [-0.5 0.5]
        #self.agentW.append(V_pred)
        #with open(path + 'omega_pred_vp' + specifier,"wb") as fp:
        #	pickle.dump(self.agentW, fp)
        
        V_pred2 = 0.25*a1[0].item() + 0.75 # >> Constrain to [0.1 0.9] >> Complete space 0 -> 1  
        self.V2 = V_pred2
        ...
        #omega_pred = 0.6*a[1].item()#Omega range : [-0.5 0.5]
        omega_pred2 = 0.6*a2[1].item()#Omega range : [-0.5 0.5]
        #self.agentW.append(V_pred)
        #with open(path + 'omega_pred_vp' + specifier,"wb") as fp:
        #	pickle.dump(self.agentW, fp)
        ...
        # Fusing
        alp = 0.5
        V_pred = ((1-alp)*V_pred1+alp*V_pred2)
        omega_pred = ((1-alp)*omega_pred1+alp*omega_pred2)
