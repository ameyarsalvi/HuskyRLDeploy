...
model = PPO.load("/home/HuskyVisServo/trained_models/AdaptiveWt/best1s",device = 'cuda')
...
class LineFollower(object):
    ...
    def camera_callback(self, data):
        ...
        im_bw = np.frombuffer(im_bw, dtype=np.uint8).reshape(105, 320, 1) # Reshape to required observation size
        im_bw_obs = im_bw
        ...
        if self.i == 1:
        	self.obs_ = inputs
        	self.enc_len = self.enc_len + self.V*0.05
        else:
        	if self.i % 25 == 0:
        		self.obs_ = inputs
        		self.enc_len = self.enc_len + self.V*0.05
        ...
        a= pred[0]
        V_pred = 0.25*a[0].item() + 0.75 # >> Constrain to [0.1 0.9] >> Complete space 0 -> 1  
        self.V = V_pred
        ...
        omega_pred = V_pred*a[1].item()#Omega range : [-0.5 0.5]
