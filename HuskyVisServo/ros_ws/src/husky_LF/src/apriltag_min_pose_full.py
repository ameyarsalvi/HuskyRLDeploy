...
Params (~):
  ...
  fx, fy, cx, cy:                      # fixed intrinsics (pixels)
...
class Node:
    def __init__(self):
        ...
        self.tag_size    = float(rospy.get_param("~tag_size", 0.162))  # meters
        self.frame_id    = rospy.get_param("~camera_frame", "camera_link")
        self.fx          = float(rospy.get_param("~fx", 615.0))
        self.fy          = float(rospy.get_param("~fy", 615.0))
        ...
        rospy.logwarn("Using FIXED intrinsics: fx=%.1f fy=%.1f cx=%.1f cy=%.1f",
    ...
    def cb(self, msg):
        ...
        for d in dets:
            ...
            # d.pose_R, d.pose_t are TAG->CAMERA. Invert to CAMERA->TAG for Pose in camera frame.
            R_tc = d.pose_R.astype(float)
            t_tc = d.pose_t.reshape(3).astype(float)
            R_ct = R_tc.T
            t_ct = - R_ct.dot(t_tc)
